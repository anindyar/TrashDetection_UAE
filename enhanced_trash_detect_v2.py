...
import math
import os
from parse_dji_srt import parse_dji_srt, get_frame_telemetry, get_altitude_feet
...
# Main Function
# -------------------------------
def main(video_path, output_video, output_json, srt_file=None,
         conf_thresh=0.6, default_altitude_ft=100, proximity_radius_ft=15):

    # Load SRT telemetry if provided
    telemetry_data = None
    if srt_file and os.path.exists(srt_file):
        print(f"[INFO] Loading SRT telemetry from: {srt_file}")
        telemetry_data = parse_dji_srt(srt_file)
        print(f"[INFO] Loaded telemetry for {len(telemetry_data)} frames")
    else:
        print(f"[INFO] No SRT file provided, using default altitude: {default_altitude_ft}ft")

    # Load YOLO models
    print("[INFO] Loading YOLOv8 models...")
    print(f"[INFO] Waste detection confidence threshold: {conf_thresh}")
    print(f"[INFO] Vehicle proximity radius: {proximity_radius_ft} feet")
    ...
    # Statistics
    all_vehicles_detected = []

    # Checkpoint interval
    checkpoint_interval = 500
    checkpoint_file = output_json + ".checkpoint"

    print("[INFO] Processing video with OPTION B: Always detect vehicles and plates...")
    frame_num = 0
    ...
    try:
        for frame_num in tqdm(range(total_frames)):
            ...
            try:
                # Get real GPS and altitude from SRT if available
                frame_telemetry = None
                if telemetry_data:
                    frame_telemetry = get_frame_telemetry(telemetry_data, frame_num + 1)  # SRT frames start at 1

                if frame_telemetry:
                    current_lat = frame_telemetry['latitude']
                    current_lon = frame_telemetry['longitude']
                    current_altitude_ft = frame_telemetry['rel_alt'] * 3.28084  # meters to feet
                else:
                    current_lat = 0.0
                    current_lon = 0.0
                    current_altitude_ft = default_altitude_ft

                # STEP 1: ALWAYS Run base model to detect all objects (cars, people, signs, etc.)
                ...
                # STEP 2: ALWAYS process vehicles and extract plates (NEW LOGIC - OPTION B)
                frame_vehicles = []
                ...
                # STEP 4: Process each waste detection and find nearby vehicles
                frame_detections = []
                    ...
                    if is_valid_waste:
                        ...
                        for vehicle_det in vehicle_detections:
                            ...
                            if real_distance_ft <= proximity_radius_ft:
                                ...
                                # Find plate info from frame_vehicles
                                for v in frame_vehicles:
                                    if v['bbox'] == vehicle_info['bbox']:
                                        vehicle_info['plate_detected'] = v['plate_detected']
                                        if 'plate_number' in v:
                                            vehicle_info['plate_number'] = v['plate_number']
                                            vehicle_info['plate_confidence'] = v['plate_confidence']
                                        break

                                nearby_vehicles.append(vehicle_info)

                                # Highlight nearby vehicle with THICK YELLOW border
                                vx1, vy1, vx2, vy2 = map(int, vehicle_det['box'])
                                cv2.rectangle(frame, (vx1, vy1), (vx2, vy2), (0, 255, 255), 5)
                                dist_label = f"NEARBY: {real_distance_ft:.1f}ft"
                                cv2.putText(frame, dist_label, (vx1, vy2 + 25),
                        ...
                        detection_entry = {
                            ...
                            "altitude_ft": round(current_altitude_ft, 2),
        ...
        print(f"[INFO] Total vehicles detected across all frames: {len(all_vehicles_detected)}")
        plates_found = sum(1 for v in all_vehicles_detected if v.get('plate_detected'))
        print(f"[INFO] License plates captured: {plates_found}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Enhanced Drone Trash Detection v2 with Always-On Vehicle Detection")
    parser.add_argument("--video", required=True, help="Path to input video file (.mp4)")
    parser.add_argument("--srt", default=None, help="Path to DJI SRT file for GPS/altitude data (optional)")
    parser.add_argument("--output_video", default="enhanced_output_v2.mp4", help="Output annotated video")
    parser.add_argument("--output_json", default="enhanced_detections_v2.json", help="Output detection JSON")
    parser.add_argument("--conf", type=float, default=0.6, help="Confidence threshold for waste detection. Default: 0.6")
    parser.add_argument("--altitude", type=float, default=100, help="Default drone altitude in feet (if no SRT). Default: 100ft")
    parser.add_argument("--radius", type=float, default=15, help="Vehicle proximity radius in feet. Default: 15ft")

    args = parser.parse_args()
    main(args.video, args.output_video, args.output_json, args.srt,
